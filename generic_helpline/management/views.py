...
class TaskObject():
    def __init__(self, task_id, task_status, tag_string, task_category, caller_name, caller_location, caller_number, client_calls, local_date, local_time, language) -> None:
        self.parent_task_id = ""
...
class newClientTasks(APIView):
    def fetch_assigns(self, assign_list):
        ...
        for assign in assign_list:
            ...
            if len(assign.action.task.language.all()) > 0:
                ...
                language = ""
            data = TaskObject(task_id, task_status, "", task_category, caller_name,
                    caller_location, caller_number, client_calls, local_date, local_time, language)
            return_list.append(data.toJSON())
        return return_list
    ...
    def get(self, request):
            ...
            pending_assigns = Assign.objects.filter(action__in=actions, status=AssignStatusOptions.PENDING)
            completed_tasks = self.fetch_assigns(completed_assigns)
            accepted_tasks = self.fetch_assigns(accepted_assigns)
            pending_tasks = list(set(self.fetch_assigns(pending_assigns)))
            data = {"accepted": accepted_tasks + completed_tasks + pending_tasks}
...
class getTask(APIView):
    ...
    def get(self, request):
        ...
        action = Action.objects.filter(task__in=tasks)
        assign = Assign.objects.filter(action__in=action)
        task = self.fetch_assigns(assign)
        # accepted_tasks = self.fetch_assigns(accepted_assigns)
        data = {"task": task}
