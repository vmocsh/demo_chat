...
def AddTaskDashboard(request):
    ...
    if request.method =='POST': 
                print(form)
                if form.is_valid():
                    name = form.__dict__["cleaned_data"]["name"]
                    phone = form.__dict__["cleaned_data"]["phone"]
                    location = form.__dict__["cleaned_data"]["location"]
                    category = form.__dict__["cleaned_data"]["category"]
                    
                    language = form.__dict__["cleaned_data"]["language"]

                    temp = Client.objects.filter(client_number=phone)
                    
                    if len(temp) > 0:
                        client = temp.first()
                    else:
                        client = Client(name=name, location=location, client_number=phone)
                        client.save()
                    cat_obj = HelperCategory.objects.filter(name=category).first()
                    try:
                        helpline = HelpLine.objects.get(name=HELPLINE)
                    except:
                        helpline = HelpLine(name=HELPLINE, helpline_number="9999999999", helpline_tollfree_number="9999999999")
                        helpline.save()
                    language_obj = Language.objects.get(language=language, helpline=helpline)
                    call_req = CallRequest(client=client, helpline=helpline)
                    call_req.save()
                    temp = Task.objects.filter(call_request__client=client, category=cat_obj)
                    
                    if len(temp) == 0:

                        task = Task(call_request=call_req, category=cat_obj)
                        task.save()
                        task.language.add(language_obj)
                        
                        action = AssignAction()
                        action.assign_action(task)
                        print("Task created")
                    else:
                        task = temp.first()
                        task.client_calls += 1
                        task.save()
                        print("Task calls updated")
                    form = AddTask()
                else :
                    print(form.errors)
    return render(request,'input.html',{ 'form':form })

def FormOrChat(request):
    # form = AddTask(request.POST or None, request.FILES or None)
    # if request.method =='POST': 
    ...
    return render(request,'form_or_chat.html')